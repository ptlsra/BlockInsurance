...
// get web3-quorum provider running at localhost port -> 8102
...
var mortgageQuotesDBUrl = "mongodb://localhost:27017/mortgage_quotes";
 ...
 */
var payPremiumEvent;
payPremiumEvent = insuranceContractDeplyed.PayPremium({}, {
    fromBlock: 'latest',
    toBlock: 'latest'
});
...
var giveQuoteEvent;
giveQuoteEvent = insuranceContractDeplyed.GiveQuote({}, {
    fromBlock: 'latest',
    toBlock: 'latest'
});
...
giveQuoteEvent.watch(function (error, result) {
    ...
    try {
        ...
        logger.error("Error in giveQuoteEvent");
    }
});
 ...
 */
var premiumEvent;
premiumEvent = insuranceContractDeplyed.InsurancePremium({}, {
    fromBlock: 'latest',
    toBlock: 'latest'
});
premiumEvent.watch(function (error, result) {
    logger.info("premiumEvent");
    logger.debug(result);
    var args = result.args;
    try {
        logger.debug("Storing transaction to DB");
        ...
        logger.error("Error in premiumEvent : "+e);
    }
});
...
app.get('/getCustomerInfo',function(request, response){
        var walletAddress = request.query.walletAddress;
            console.log("-----------------Get Customer Information By Wallet Address -----------------------");
            var result = (deployedContract['getCustomer'](walletAddress,{from: String(walletAddress), gas: 4000000}));
...
app.post('/updatePortfolioStatus', function (request, response) {
    ...
    try {
        var transactionId = (deployedContract['updatePortfolioStatus'](sharedOwnerAddress, loanId, status, message, {
            from: String(insuranceAccountAddress),
            gas: 4000000
        }));
...
app.post('/addSharedOwner', function (request, response) {
    ...
    try {
        var transactionId = (deployedContract['addSharedOwner'](owner, loanId, newOwnerAddress, {
            from: String(owner),
            gas: 4000000
        }));
...
app.get('/getDocumentHashByLoanId', function (request, response) {
    ...
    logger.debug("printing response " + JSON.stringify(customerDocList));
...
app.post('/createInsuranceCompany', function (request, response) {
    ...
    let promiseA = new Promise((resolve, reject) => {
        let wait = setTimeout(() => {
            try {
                var jsonResponse = (insuranceContractDeplyed['createInsuranceCompany'](companyName, insuranceAccountAddress, {
                    from: String(insuranceAccountAddress),
                    gas: 4000000
                }));
                ...
                logger.error("Error in creating insuranceCompany");
            }
        }, 7000)
    });
});
...
app.get('/getInsuranceCompanies',function(request, response){
    ...
    logger.info("getInsurnaceCompanies");
...
app.post('/requestForQuote',function(request, response){
    ...
    logger.debug("printing insurance addresses : "+insuranceList);
...
app.get('/getCustomerPortfolioInfo', function (request, response) {
    ...
    var portfolioMortgageList = (deployedContract['getPortfolioMortgage'](loanId, {
        from: String(walletAddress),
        gas: 4000000
    }))
    var portfolioPropertyList = (deployedContract['getPortfolioProperty'](loanId, {
        from: String(walletAddress),
        gas: 4000000
    }))
...
app.post('/giveQuoteToCustomer', function (request, response) {
    ...
    try {
        ...
        var transactionId = insuranceContractDeplyed['giveQuoteToCustomer'](loanId, quote, insuranceAddress, contractAddress, premium, {
        ...
        logger.error("Error in giveQuoteToCustomer");
    }
});
...
app.get('/getPremiumAmount', function (request, response) {
    ...
    try {
        ...
        logger.debug("premiumDetails : " + premiumDetails);
        var jsonResponse = {
            "loanId": loanId,
        ...
        logger.error("Error in getPremiumAmount");
    }
});
...
app.get('/getQuote', function (request, response) {
    ...
    logger.debug("printing get quote response");
...
app.post('/payPremium', function (request, response) {
    ...
    fs.readFile('Insurance.pdf', function (err, data) {
        ...
        ipfs.files.add(files, (err, filesAdded) => {
            ...
            var transactionId = insuranceContractDeplyed['payPremium'](loanId, premiumAmount, insuranceAddress, contractAddress, userName, customerAddress, filesAdded[0].hash, {
                from: String(insuranceAddress),
                gas: 4000000
            });
            var updateRequestQuoteTx = deployedContract['updateRequestQuote'](loanId, insuranceAddress, "approved", {
                from: String(insuranceAddress),
                gas: 4000000
            });
            var addOwnerTx = deployedContract['addSharedOwner'](customerAddress, loanId, insuranceAddress, {
                from: String(insuranceAddress),
                gas: 4000000
            });
            var updatePortfolioTx = deployedContract['updatePortfolioStatus'](customerAddress, loanId, "Insurance_approved", "ready_to_disburse", {
                from: String(insuranceAddress),
                gas: 4000000
            });
    ...
    try {
        ...
        for (var index = 0; index < loanIds.length; index++) {
            ...
            if (quoteStatus[1] == "requesting_quote") {
                logger.debug("*********** checking for  loanId " + loanIds[index]);
                ...
                for (var index2 = 0; index2 < customerList.length; index2++) {
                    if (userName == customerList[index2]) {
                        ...
                        logger.debug("printing insurance list for loanId " + loanId + " insurance list is : " + insuranceList);
                        ...
                        for (var index3 = 0; index3 < rLength; index3++) {
                            ...
                            if (quoteObject[1] == "rate_of_interest_is_4") {
                                found = 1;
                                break;
                            }
                        }
                        ...
                        if (found == 0) {
                            ...
                            var giveQuoteTxId = insuranceContractDeplyed['giveQuoteToCustomer'](loanId, "rate_of_interest_is_4", insuranceAccountAddress, contractAddress, "85", {
                                from: String(insuranceAccountAddress),
                                gas: 4000000
                            });
                            logger.debug("Giving quote to customer with  transactionId : " + giveQuoteTxId);
                            logger.debug("******* updating portfolio status ***********");
                            var updatePortfolioStatusTxId = (deployedContract['updatePortfolioStatus'](bankAddress, loanId, "insurance_quote_approved", "accept_quote", {
                                from: String(insuranceAccountAddress),
                                gas: 4000000
                            }));
        ...
        logger.error("Error in autoQuoteApproval : " + e);
    }
}


 setInterval(function(){
        logger.debug("calling autoQuoteProvider() method");
        autoQuoteProvider();
...
function storeTransactionForInsurance(collectionName, tx_id, insuranceAddress, description, loanId){
    console.log("***************** store transactions to database *******************");
        ...
        console.log("printing loanId "+ loanId);
...
app.get('/getTransactionsForInsurance',function(request, response){
    console.log("******************** get transactions for insurance ****************");
...
function storeTransaction(user_name,  tx_id, description, accountAddress, loanId){
    ...
    try {
        ...
        var result = (deployedContract['getCustomer'](accountAddress, {
        ...
        logger.error("Error in storing transaction");
    }
}
...
app.get('/getAllTransactions', function (request, response) {
    ...
    try {
        ...
        logger.error("Error in getAllTransactions");
    }
});
...
app.get('/getAllCustomerLoans', function (request, response) {
    ...
    try {
        ...
        mortgageDB.listCollections().toArray(function (err, result) {
            ...
            for (var index = 0; index < result.length; index++) {
                ...
                mortgageDB.collection(collectionsName).find({}).toArray(function (err, record) {
                    ...
                    var customerPortfolios = (deployedContract['getPortfolios'](walletAddress));
                    var length = customerPortfolios[1].length;
                    var customerObject = (deployedContract['getCustomer'](walletAddress, {
                        from: String(walletAddress),
                        gas: 4000000
                    }));
        ...
        logger.error("Error in storing transaction : " + e);
    }
});
...
app.get('/getCustomerTransactionsByLoanId', function (request, response) {
    logger.info("getCustomerTransaction");
    var userName = request.query.userName;
    var loanId = parseInt(request.query.loanId);
    logger.debug("userName is " + userName);
    logger.debug("loan id is " + loanId);
    ...
    mortgageTxnDB.collection(collectionsName).find({
        loanId: loanId
            ...
            MongoClient.connect(mortgageAutoDBUrl, function(err, mortgageAutoDBTemp) {
                ...
                MongoClient.connect(mortgageQuotesDBUrl, function(err, mortgageQuoteDBTemp) {
                    ...
                    app.listen(appPort, '0.0.0.0',function () {
                        logger.debug("Application  listening on port "+appPort+". ");
                    })
                });
            });
        });
